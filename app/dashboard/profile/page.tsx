...
export default function ProfilePage() {
  ...
  return (
    <div className="max-w-2xl animate-slide-up">
      <h1 className="text-xl font-bold text-[var(--dash-text)] mb-6">Профіль</h1>

      <div className="bg-white rounded-2xl border border-[var(--dash-border)] p-6 mb-6">
        <div className="flex items-center gap-5">
          {user.avatar ? (
            <img src={user.avatar} alt="" className="w-20 h-20 rounded-2xl object-cover" />
          ) : (
            <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-[var(--dash-accent)] to-[var(--dash-accent-light)] flex items-center justify-center text-white font-bold text-2xl">
              {initials}
            </div>
          )}
          <div>
      ...
      <div className="bg-white rounded-2xl border border-[var(--dash-border)] p-6">
        ...
        <form onSubmit={handleSave} className="space-y-5">
          ...
          <button type="submit" disabled={saving}
            className="px-5 py-2.5 rounded-xl bg-[var(--dash-accent)] hover:bg-[var(--dash-accent-hover)] disabled:opacity-50 text-white font-semibold text-sm transition-colors">
      ...
      {user.is_executor && (
        <div className="bg-white rounded-2xl border border-[var(--dash-border)] p-6 mt-6">
